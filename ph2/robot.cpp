...
float Robot::estimation_position(int* bag_of_features,float likelihood){

float fused_observation=1.0;
int i;
for(i=0;i<ROWS;i++)
fused_observation*=bag_of_features[i];
fused_observation/=ROWS;

return fused_observation*likelihood;
...
pcl::io::savePCDFileASCII("test_rfid.pcd",cloud);
float s=0;
for(i=0;i<numberOfParticle;i++)
s=s+weight(i);
...
void Robot::compute_weight_toto(MatrixXd cloud_particles,VectorXd tag){
...
pcl::io::savePCDFileASCII("test_rfid.pcd",cloud);
//rf.RFID_cluster_data_with_pcl();

}


float Robot::compute_weight(MatrixXd all_tags){
// rf;
float x,y,d=0.0,i,angle_offset,angle;
ofstream likeilihood("likelihood.txt");
x=X_predicted(0);
y=X_predicted(1);
float likelihood;
float likelihood_of_all_tags=1.0;
for(i=0;i<numberOfTags;i++){
angle_offset=3*pi/4;
angle=3*pi/8;
d=sqrt((x-all_tags(i,0))*(x-all_tags(i,0))+(y-all_tags(i,1))*(y-all_tags(i,1)));

likeilihood<<d<<endl;
if((isInSector(all_tags(i,0),all_tags(i,1),angle_offset,angle, x, y)==true)){
...
if (d<0.3)
   ...
   // likelihood_of_all_tags*=likelihood;
    }


return likelihood_of_all_tags;
}
